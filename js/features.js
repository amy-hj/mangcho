...
calendarMarkup = function () {
  ...
  var cnt = Object.keys(FEAT.attendance).filter(function (k) { return k.indexOf(y + '-' + String(m).padStart(2, '0')) === 0; }).length;
  return '' +
    '<div class="calendar">' +
      '<div class="cal-month">' +
        '<button class="cal-arrow" data-cal="prev"><img src="' + ASSET.calPrev + '" alt="이전 달"></button>' +
        '<span class="cal-title">' + y + '년 ' + m + '월 <small class="cal-cnt">출석 ' + cnt + '일</small></span>' +
...
scAttendDone = function () {
  return '<div class="screen screen--paper">' + attendBackdrop() +
    ...
    '<div class="modal modal--attend"><img class="m-stamp" src="' + ASSET.stamp + '" alt="">' +
...
chatListMarkup = function (rooms) {
  var items = rooms.map(function (r) {
    return '<button class="list-item" data-room="' + r.id + '">' +
      ...
      '<div class="body">' +
        '<div class="nm-wrap"><span class="nm">' + esc(r.name) + '</span>' + (r.pinned ? '<img class="pin" src="' + ASSET.pinFilled + '" alt="고정됨">' : '') + '</div>' +
