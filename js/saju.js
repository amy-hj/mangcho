...
/* ============================================================
   ...
   ============================================================ */
function sajuForm() {
  return (typeof FEAT !== 'undefined' && FEAT.form) ? FEAT.form : (typeof state !== 'undefined' && state.form ? state.form : PROFILE_FORM);
}

function scSaju(state) {
  var f = sajuForm(), r;
  try { r = SAJU.calc(f); } catch (e) { return '<div class="screen screen--paper">' + statusBar() + subBar('my', '내 명반') + '<p class="sj-err">생년월일을 먼저 입력해주세요.</p></div>'; }
...
function scZiwei(state) {
  var f = sajuForm(), a;
  try { a = SAJU.ziwei(f); } catch (e) { return '<div class="screen screen--paper">' + statusBar() + subBar('my', '내 명반') + '<p class="sj-err">생년월일을 먼저 입력해주세요.</p></div>'; }
  ...
  return '' +
    '<div class="screen screen--paper">' + statusBar() + subBar('my', '내 명반') +
      ...
      '<div class="zw-wrap"><div class="zw-grid">' + grid + '</div></div>' +
      '<p class="zw-legend">사화: <i class="zw-mut zw-mut--록">록</i><i class="zw-mut zw-mut--권">권</i><i class="zw-mut zw-mut--과">과</i><i class="zw-mut zw-mut--기">기</i> · 주성/<small>보조성</small>/<small class="dim">잡성</small></p>' +
    '</div>';
}
