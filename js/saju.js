...
var SAJU = (function () {
  ...
  function fortune(form, date) {
    ...
    /* 행운 아이템: 내 일간 오행 × 오늘 지지 오행 조합 → 사람마다·날마다 다름 */
    var _ord = ['木', '火', '土', '金', '水'];
    var wx = _ord[(_ord.indexOf(GAN_WX[natal.dayGan]) + _ord.indexOf(ZHI_WX[todayZhi] || '土') + 5) % 5];
    return {
...
function scZiweiInner(state) {
  ...
  return '' +
    '<div class="screen screen--paper">' + statusBar() + subBar('my', '내 명반') +
      ...
      '<p class="zw-legend">사화: <i class="zw-mut zw-mut--록">록</i><i class="zw-mut zw-mut--권">권</i><i class="zw-mut zw-mut--과">과</i><i class="zw-mut zw-mut--기">기</i> · 주성/<small>보조성</small>/<small class="dimtxt">잡성</small></p></div>' +
    '</div>';
}
